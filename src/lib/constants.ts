...
export const CHANGELOG = [
  "Versão 8.9b - Blindagem de Segurança Institucional, Biometria/Face-ID & Estabilização Anti-Crash",
  "Autenticação Biométrica e Face-ID via WebAuthn no Portal do Aluno com opção 'Usar a Biometria ou Face-ID' e fallback por PIN de 4 dígitos; proteção e detecção de janelas embutidas (iframe) para segurança de credenciais biométricas; blindagem contra tela branca com suspensão de recargas forçadas em iframes e tratamento defensivo no carregamento assíncrono de módulos; remoção integral e definitiva de senhas mestras hardcoded e de privilégios por domínio de e-mail conforme o SEGURANCA.md; admissão de administradores restrita exclusivamente a convites validados criptograficamente pelo servidor (/api/admin/accept-invite); eliminação da regressão de versões antigas (versões fantasmas como v6.9b) através de particionamento e versionamento isolado de namespaces de CacheStorage do Service Worker (app-shell-cache-v8.9b); e otimização de performance eliminando gargalos na thread principal.",
  ...
  "Eliminação de travamentos ao clicar em botões com execução assíncrona de efeitos sonoros e feedback tátil em memória; sincronização automática em tempo real de novas versões via service worker, Firestore e version.json estático; bloqueio estrito contra o funcionamento em versões obsoletas sem ocorrência de loops de recarregamento.",
