...
const normalizeString = (str) => str.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9 ]/gi, "").toLowerCase().replace(/\s+/g, ' ').trim();
...
function matchStudent(cleanName) {
  ...
  // Custom overrides for specific names mentioned by the user
  ...
  if (cleanParam === 'joao pedro') cleanParam = 'joao pedro'; // Already fine
  ...
  for (const s of validStudents) {
     ...
     if (score > 0) {
        // Specific user request handling: Leonardo is religioso, Ferreira is Diocese de Marília
